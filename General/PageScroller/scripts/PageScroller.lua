...
function PageScroller:new(layer, size, pad, duration, cb)
  local newScroller = {}
...
function PageScroller:release(x)
  if (self.pressed == true) then
    ...
    -- show animation step
    adata["to"] = self.pad
    adata["key"] = self.layer.."."..self.controls[show]..".grd_x"
    gre.animation_add_step(self.anim, adata)
    
    cur = gre.get_value(self.layer.."."..self.controls[hide]..".grd_x")
    if (cur < 0) then
    ...
    -- hide animation step
    adata["to"] = target
    adata["key"] = self.layer.."."..self.controls[hide]..".grd_x"
    gre.animation_add_step(self.anim, adata)
...
function PageScroller:drag(x)
  if (self.pressed == true) then
    ...
    data[self.layer.."."..self.controls[self.pos]..".grd_x"] = delta + self.pad
    if (delta < 0) then
      local n = self.pos + 1
      if (n > #self.controls) then
        n = 1
      end
      data[self.layer.."."..self.controls[n]..".grd_x"] = data[self.layer.."."..self.controls[self.pos]..".grd_x"] + self.width
    else
      local n = self.pos - 1
      if (n <= 0) then
        n = #self.controls
