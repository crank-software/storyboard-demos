...
gst-launch-1.0 uridecodebin expose-all-streams=false uri=file:///crank/testApps/video/forest_run_m.mp4 caps="video/x-h264;audio/x-raw" . ! queue ! h264parse ! queue ! g1h264dec ! video/x-raw,width=800,height=480 ! g1fbdevsink device=/dev/fb0
rc=$?
if [ "$rc" -eq 0 ]; then
	echo "IOGEN"
