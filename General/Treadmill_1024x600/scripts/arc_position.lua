...
-- STORYBOARD DEMO: Turn off the cell resize and repositioning
--  if(true) then
--    return
--  end

  local tableName = mapargs.context_control
  local guideName = tableName .. "Guide"
  ...
  for r=1,tInfo.rows do
    ...
    local newX = resolve_x_position_from_y(guideName, cellInfo.y + (cellInfo.height / 2)) - 505

    -- Arbitrarily scale the width of the cell based on the position
    local newWidth = 1024-newX-505
