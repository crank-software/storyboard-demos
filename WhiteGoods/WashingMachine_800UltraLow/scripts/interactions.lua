local pressed = 0
local sliderButtonSize = 90
local sliderButtonSizeMid = 175
local topSnapOffset = 210
local pressedPos
local curPos
local startPressPos
...
local startBounds = {150,290}
local customizeBounds = {300, 340}
...
local function setupCycleSignal(incState)
  ...
  if(incState == 1)then
    ...
    data["homeAbout.cycleSignal_group.switchFG.grd_x"] = 102
  else
    data["homeAbout.cycleSignal_group.on.grd_hidden"] = 1
    data["homeAbout.cycleSignal_group.off.grd_hidden"] = 0
    data["homeAbout.cycleSignal_group.switchFG.grd_x"] = 68
  end
  gre.set_data(data)
end
