...
local g_scale_rpm = 0.4
local g_scale_bpm = 0.4
...
function update_small_graphs ( mapargs )
  ...
  v["home_layer.2_group.trend.trendFillData"] = ecgPoints  .. " 1:83 6:83 369:83 374:83 477:83 "
...
function plot(mapargs)
  ...
  local oxiPointData = {}
  for i = 1, n, 1 do
    point = string.format("%.0f",i*3)..":"..string.format("%.0f", ecgSmall[i]*0.5)
    table.insert(ecgPointData, point)

    point = string.format("%.0f",i*3)..":"..string.format("%.0f", plethSmall[i]*0.5)
