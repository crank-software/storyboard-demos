...
local settings_IntervalCheckToggle = 1
...
local goToScreen
local lastScreen = "navigation_layer.home"
local heldID


function navigatePress(mapargs)
  local currentScreen = mapargs.context_screen
  local currentScreenControl = mapargs.context_control
  goToScreen = gre.get_value(mapargs.context_control..".screen")
  ...
  gre.set_value("newScreen", goToScreen)

  if(currentScreen == 'homeScreen')then
    gre.animation_trigger('TRANSITION_homeHide')
  elseif(currentScreen == 'crankScreen')then
    gre.animation_trigger('TRANSITION_crankHide')
  elseif(currentScreen == 'patientScreen')then
    gre.animation_trigger('TRANSITION_patientHide')
  elseif(currentScreen == 'settingsScreen')then
    gre.animation_trigger('TRANSITION_settingsHide')
  end
  ...
  local animData = {}
  animData['context'] = currentScreenControl
  animData['id'] = id
  gre.animation_trigger('App_bottomNav', animData)
  
  animData['context'] = lastScreen
  animData['id'] = id+1
  gre.animation_trigger('App_bottomNavClear', animData)
  lastScreen = currentScreenControl
  heldID = id
  
  print(currentScreen, goToScreen)
end
...
local function updateColoursLight()
    ...
    data["homeScreen.temp_colour"] = 0x353555
    data["settings_layer.intervals.ring_bg_colour"] = 0xD4D4D4
    data["white_black"] = 0xFFFFFF
    data["black_white"] = 0x000000
...
local function updateColoursDark()
    ...
    data["settings_layer.intervals.ring_bg_colour"] = 0x111111
    data["white_black"] = 0x000000
    data["black_white"] = 0xFFFFFF
...
local function updateImages(folder)
  ...
  data["crank_layer.trial"] = folder .."laptop.png"
...
--function toggle_animation(mapargs) 
--  local key = "nielsen_layer.logo_control.toggle"
--  local toggle = gre.get_value(key)

function ToggleSkin(mapargs)
    local skin = gre.get_value("navigation_layer.skinSwitch.skin")
    local key = "navigation_layer.skinSwitch.key"
    local toggle = gre.get_value(key)
    ...
    if skin == "light" then
        ...
        gre.set_data(data)
        gre.animation_trigger("light_to_dark2")
        skin = "dark"
    elseif skin == "dark" then
        local data = {}
        toggle = 0
        ...
        gre.set_data(data)
        gre.animation_trigger("light_to_dark2_reversed")
        skin = "light"
    end
    updateUI(skin)
end
...
function TRENDS_toggleAlarm(mapargs) 
  local toggle = gre.get_value(mapargs.context_group..".alarmIcon.toggle")
  local newToggle
  local animData = {}
  animData['context'] = mapargs.context_group
  animData['id'] = gre.get_value(mapargs.context_group..".id")
  if(toggle == 0)then
    gre.animation_trigger('HOME_alarmOn', animData)
    newToggle = 1
  else
    gre.animation_trigger('HOME_alarmOff', animData)
    newToggle = 0
  end
  gre.set_value(mapargs.context_group..".alarmIcon.toggle", newToggle)
end

local currentSettings = "settings_layer.right_controls.buttonbg_1"
--- @param gre#context mapargs
function SETTINGS_navigationPress(mapargs) 
  local animData = {}
  local pressedSettings = mapargs.context_control 
  local id = math.random(1,1000)
  animData['context'] = pressedSettings
  animData['id'] = id
  gre.animation_trigger('SETTINGS_pressSideButton', animData)
  animData['context'] = currentSettings
  animData['id'] = id + 1
  gre.animation_trigger('SETTINGS_unselectSideButton', animData)
  currentSettings = pressedSettings
end


--- @param gre#context mapargs
function updateTrendsText(mapargs) 
  local BP1 = gre.get_value("home_layer.1_group.bpHigh")
  local BP2 = gre.get_value("home_layer.1_group.bpLow")
  local temp = math.floor(gre.get_value("home_layer.4_group.temp") *10) / 10
  local o2 = gre.get_value("home_layer.3_group.oNumber")
  local heartrate = gre.get_value("home_layer.2_group.bpRate")
  
  local data = {}
  data["home_layer.1_group.value.text"] = BP1.."/"..BP2
  data["home_layer.3_group.value.text"] = o2
  data["home_layer.4_group.value.text"] = temp..'°'
  data["home_layer.2_group.value.text"] = heartrate
  gre.set_data(data)
end
