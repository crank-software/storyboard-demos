...
function update_numbers()
  ...
  local data = {}
  local oxiText = math.random(92, 97)
  local ecgNum = math.random(96, 104)
  local temp = math.random(979,985)/10
  local upper = math.random(116,123)
  local lower = math.random(78,86)
  if(string.len(temp)==2)then
    temp = temp..".0"
  end
  
  data["home_layer.3_group.value.text"] = math.floor(oxiText).."%"
  data["home_layer.2_group.value.text"] = math.floor(ecgNum)
  data["home_layer.4_group.value.text"] = temp.."°"
  data["home_layer.1_group.value.text"] = upper.."/"..lower
  gre.set_data(data)
end
