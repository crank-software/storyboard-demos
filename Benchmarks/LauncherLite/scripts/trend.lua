...
function update_small_graphs ( mapargs )
  ...
  v["trendsMovement.2_group.trend.trendData"] = ecgPoints
  v["trendsMovement.2_group.trend.trendFillData"] = ecgPoints  .. " 1:134 6:140 369:140 374:134 477:134 "
  
  gre.set_data(v)

  local dk_data = {}
  
  dk_data = gre.get_data("trends.dia", "trends.oxi", "trends.pulse", "trends.sys", "trends.temp")
  
  local diaText = dk_data["trends.dia"]
  local oxiText = dk_data["trends.oxi"]
  local pulText = dk_data["trends.pulse"]
  local sysText = dk_data["trends.sys"]
  local temText = dk_data["trends.temp"]

  local data = {}
  
  data["trendsMovement.1_group.value.text"] = sysText.."/"..diaText
  data["trendsMovement.2_group.value.text"] = pulText
  data["trendsMovement.3_group.value.text"] = oxiText
  
  data["trendsMovement.4_group.value.text"] = string.sub(temText, 0, 4)
  gre.set_data(data)
end
...
function plot(mapargs)
  ...
  local ecgPoints = table.concat(ecgPointData, " ") 
  data["trendsMovement.2_group.trend.trendData"] = ecgPoints
  data["trendsMovement.2_group.trend.trendFillData"] = ecgPoints
  
  local oxiPoints = table.concat(oxiPointData, " ") 
  data["trendsMovement.3_group.trend.trendData"] = oxiPoints
  data["trendsMovement.3_group.trend.trendFillData"] = oxiPoints
