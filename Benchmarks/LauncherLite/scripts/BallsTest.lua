...
function BallsTest:moveBalls() 
	...
	local prefix = "main_layer.ball" 
	local stem
	local db_updates = {}
	for i=0,self._N-1,1 do
		stem = prefix .. tostring(i)
		--print(stem .. " --> " .. tostring(self._balls[i]._x) .. "," .. tostring(self._balls[i]._y)) 
		db_updates[stem .. ".grd_x"] = self._balls[i]._x
		db_updates[stem .. ".grd_y"] = self._balls[i]._y
	end
	gre.set_data(db_updates)	
end
...
function BallsTest:showFPS() 
	...
	local db_update = {}
	db_update["fpsText"] = "FPS: " .. tostring(math.floor(fps))
	gre.set_data(db_update)             
end
